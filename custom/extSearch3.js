//engine: JScript
//uname: ExtendedSearch3
//dname: Расширенный поиск 3
...
////////////////////////////////////////////////////////////////////////////////////////
////{ Cкрипт "Расширенный поиск 3" (extSearch3.js) для проекта "Снегопат"
////
...
////////////////////////////////////////////////////////////////////////////////////////
////{ ExtSearch - Расширенный поиск 3 в тексте модуля.
////
...
var ProcessAreas = {
    PROCESS_ATCLIENT   : '\s*(AtClient|НаКлиенте)\s*',
    PROCESS_ATSERVER   : '\s*(AtServer|НаСервере)\s*',
    PROCESS_ATSERVERNC : '\s*(AtServerNoContext|НаСервереБезКонтекста)\s*'
}
...
ExtSearchDialog = ScriptForm.extend({
    ...
    construct : function (query, initSearchArea) {	
        ...
        this.form.КлючСохраненияПоложенияОкна = "extSearch3.dialog.js"
        this.loadSettings();
	...
	getSearchQueryParams: function () {
		...
		params.Insert('ProcessArea', 	this.form.ProcessArea); // Павлюков С.Ю.
		return params;
	},
...
ExtSearch = ScriptForm.extend({
    ...
    construct : function (isExtend) {
        ...
        var pathToForm=SelfScript.fullPath.replace(/.js$/, '.results.ssf')
		this._super(pathToForm);
                
        this.form.КлючСохраненияПоложенияОкна = "extSearch3.js"
	...
	setQuery : function (searchQueryParams) {
        ...
        this.form.ProcessArea	= searchQueryParams.ProcessArea;//Павлюков С.Ю.
    ...
    search : function (obj, re) {
        ...
        if(!re.multiline)
        {
			var curArea = "В общем";//Павлюков С.Ю.
            var curMethod = { 
			...
			//Павлюков С.Ю.
			var isSuitableArea = (this.form.ProcessArea == ProcessAreaValues.AnyWhere);
            ...
            for(var lineIx=0; lineIx < lines.length; lineIx++)
            {
                ...
                {
					curArea = "НаКлиенте"
					if(this.form.ProcessArea == ProcessAreaValues.AnyWhere || this.form.ProcessArea == ProcessAreaValues.AtClient)
						isSuitableArea = true
					else
				...
				else
				{
					...
					{
						curArea = "НаСервере"
						if(this.form.ProcessArea == ProcessAreaValues.AnyWhere || this.form.ProcessArea == ProcessAreaValues.AtServer)
							isSuitableArea = true
						else
							isSuitableArea = false
					}
					else
					{
						...
						if (matches && matches.length)
						{
							curArea = "НаСервереБезКонтекста"
							if(this.form.ProcessArea == ProcessAreaValues.AnyWhere || this.form.ProcessArea == ProcessAreaValues.AtServerNC)
								isSuitableArea = true
							else
								isSuitableArea = false
						}
    ...
    getRowForTheCurrentLine: function(docRow) {
		...
		var curObj = docRow._object;
		
		var twnd = curObj.activate();
		try
		{
			if (twnd==undefined)
			{
				curObj.obj.editProperty(curObj.prop.id);
				curObj.obj.openModule(curObj.prop.id);
				//curObj.obj.parent.openModule(curObj.prop.id);
				twnd = GetTextWindow();
			}
		}
		catch (e){}
    ...
    addSearchResult : function (docRow, line, lineNo, matches, methodData) {
        ...
        if(undefined != methodData)
		{
            ...
            resRow.Area = methodData.Area;//Павлюков С.Ю.
		}
    ...
    goToLine : function (row) {
		...
		try
		{
			if (targetWindow==undefined)
			{
				...
				//row._object.parent.openModule(row._object.prop.id);
    ...
    CmdBar_BtAbout : function (control) {
        RunApp('http://snegopat.ru/scripts/wiki?name=extSearch3.js');
    },
    ...
    SetControlsVisible : function() {
        ...
        this.form.caption = "Расширенный поиск 3 в модуле";
    }
...
ExtSearchGlobal = ExtSearch.extend({
    ...
    construct : function () {
        ...
        for(var i = 0; i < hotkeys.count; i++)
        {
            ...
            if (Команда.indexOf("ExtendedSearch3::Отменить глобальный поиск")!=-1){
    ...
    searchInMetadata : function(fromHotKey){
        ...
        if (this.dynamicHotKey) 
            hks.AddHotKey("Ctrl+Shift+BkSpace", "ExtendedSearch3", "Отменить глобальный поиск");
    ...
    SetControlsVisible : function(){
        ...
        if (this.isGlobalFind){
            this.form.caption = "Расширенный поиск 3 в модуле (глобальный)";
        }
    }, 
